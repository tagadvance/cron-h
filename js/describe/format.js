...
export function createFormat(locale) {
	...
	const calendar = new Intl.DateTimeFormat(locale, { month: 'long', day: 'numeric', timeZone: 'UTC' });
	...
	return {
		...
		weekdays: (values, style = 'long') =>
			conjunction.format(values.map((day) => weekday[style].format(REFERENCE_SUNDAY + day * 86400000))),
		...
		months: (values) => conjunction.format(values.map((m) => month.format(Date.UTC(2024, m - 1, 1))))
	};
}
